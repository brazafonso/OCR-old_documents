...
--------------------------------------------------------------
    ...
    parser.add_argument('-ot','--output_type'           ,type=str,nargs='*' ,default=['markdown']                       ,help='Output type. Possible values: markdown, html, txt (default: markdown).', choices=['markdown','txt','txt_simple'])
    parser.add_argument('-tc','--text_confidence'       ,type=int,nargs='?'   ,default=10                               ,help='Text confidence level. Possible values: 0-100.')
    parser.add_argument('-tt','--target_type'           ,type=str,nargs=1   ,default='newspaper'                        ,help='Target type. Possible values: newspaper, simple.',choices=['newspaper','simple'])
...
Available models:
    ...
    parser.add_argument('--calibrate'                ,type=str,nargs='*'    ,default=[]                                                ,help='Calibrate pipeline by specifying a folder with a target and results for comparison. A folder for pipeline configs can also be given. By default, uses pipeline config folder in validation folder.',action=CustomAction_calibrate)
